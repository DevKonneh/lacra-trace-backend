...
def register_expire_transactions_task(apps, schema_editor):
    ...
    PeriodicTask.objects.get_or_create(
        ...
        task="lacra.contrib.tasks.transactions.expire_transactions",
        crontab=crontab,
    )
...
def remove_expire_transactions_task(apps, schema_editor) -> None:
    PeriodicTask = apps.get_model("django_celery_beat", "PeriodicTask")
    PeriodicTask.objects.filter(task="lacra.contrib.tasks.transactions.expire_transactions").delete()
...
def register_cleanup_unverified_gadgets_task(apps, schema_editor):
    ...
    PeriodicTask.objects.get_or_create(
        ...
        task="lacra.contrib.tasks.cleanup.cleanup_unverified_gadgets",
        crontab=crontab,
    )
...
def remove_cleanup_unverified_gadgets_task(apps, schema_editor) -> None:
    PeriodicTask = apps.get_model("django_celery_beat", "PeriodicTask")
    PeriodicTask.objects.filter(task="lacra.contrib.tasks.cleanup.cleanup_unverified_gadgets").delete()
...
def register_distribute_transactions_over_seasons_task(apps, schema_editor) -> None:
    ...
    PeriodicTask.objects.get_or_create(
        ...
        task="lacra.contrib.tasks.season_distribution.distribute_transactions_over_seasons",
        crontab=crontab,
    )
