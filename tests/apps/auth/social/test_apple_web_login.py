...
class TestAppleWebLogin:
    URL = reverse("apple_web_login")
    ...
    def test_oauth_parsing_success(self, client: APIClient, mocker: MockerFixture) -> None:
        ...
        mock_apple_client.fetch_access_token.return_value = {"access_token": "test-token", "id_token": "test-id-token"}
        ...
        mock_apple_client.fetch_access_token.assert_called_once_with(
            code="valid-code",
            redirect_uri="http://localhost:3000/oauth/callback",
            state="test-state",
            nonce="test-nonce",
        )
        mock_apple_client.parse_id_token.assert_called_once()

    def test_oauth_parsing_error(self, client: APIClient, mocker: MockerFixture, snapshot: SnapshotAssertion) -> None:
        mock_oauth = MagicMock()
        mock_apple_client = MagicMock()
        mock_apple_client.fetch_access_token.side_effect = OAuthError
        mock_oauth.apple = mock_apple_client
